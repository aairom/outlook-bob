...
4. **Choose export format** — Recipients CSV · Emails CSV · EML Files · JSON
5. **Select fields** *(CSV / JSON / EML)* — From · To/CC · Subject · Body · Attachments
...
|---|---|---|
| Recipients CSV | `output/recipients_TIMESTAMP.csv` | Unique addresses + display names |
| Emails CSV | `output/emails_TIMESTAMP.csv` | One row per message, selected fields |
| EML Files | `output/eml_export_TIMESTAMP/<Folder>/` | One `.eml` per message |
| JSON | `output/emails_TIMESTAMP.json` | Structured array of message objects |
